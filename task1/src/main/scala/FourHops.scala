//------------------------------------------------------------------------------
// <YOUR_FULL_NAME_HERE>
// THOMAS THEO NEHEMIE BENCHETRIT
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
    ...
    */
    def countVertices(graph: GRAPH): Int =  graph.flatMap(_.productIterator).distinct().collect().size
//------------------------------------------------------------------------------
//  MapReduce
//------------------------------------------------------------------------------
    /*
    ...
    */
    def mmMapper(N: Int, index: (Int, Int)): List[((Int, Int), (Int, Int))] = {
        (for(k <- 0 to N-1;id <- 0 to 1)
        yield {
            if (id ==0) ((k,index._2),(id,index._1)) else ((index._1,k),(id,index._2)) 
        }).toList
    }
...
//------------------------------------------------------------------------------
    ...
    */
    def mmReducer(productElements: ((Int, Int), Iterable[(Int, Int)])): (Int, Int, Int) = {
        ...
        val elemB = productElements._2.filter(_._1==0).map(_._2).toSet
        val elemA = productElements._2.filter(_._1==1).map(_._2).toSet
        val value = elemA.intersect(elemB).size
        if (value > 0) (i,j,1) else (i,j,0)
...
//------------------------------------------------------------------------------    
    ...
    */
    def matrixMultiply(matrix: GRAPH, N: Int): GRAPH = {
        matrix.flatMap(x => mmMapper(N,x)).groupByKey().map(mmReducer).filter(_._3 !=0).map(x => (x._1,x._2))
    }
